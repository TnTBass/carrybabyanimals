...
- The public changelog style gate enforces that player/server-admin framing mechanically. If `CHANGELOG.md` mentions mixins, payload classes, task numbers, implementation plans, or internal planning paths, rewrite the note as visible gameplay, admin/config impact, compatibility, or a player-facing fix.
