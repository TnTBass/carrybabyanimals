...
public final class NeoForgeCarriedBabyRenderState {
    public static final ContextKey<Boolean> SUPPRESS_VANILLA_RENDER = new ContextKey<>(
            Identifier.fromNamespaceAndPath(CarryBabyAnimals.MOD_ID, "suppress_carried_baby_render")
    );
    ...
    private NeoForgeCarriedBabyRenderState() {
    }
    ...
    public static boolean startLocalReaction(
        ...
        LOCAL_REACTIONS.put(babyEntityId, new LocalReactionState(reactionType, startTick, durationTicks));
        return true;
    }
    ...
    public static void pruneMissingEntities(IntPredicate entityExists) {
        BABY_TO_CARRIER.entrySet().removeIf(entry -> pruneMissingCarry(entry, entityExists));
    }
    ...
    public record LocalSleepyVisualState(long sleepyStartTick, long asleepStartTick) {
        ...
        static LocalSleepyVisualState fromDelay(long sleepyStartTick, int asleepDelayTicks) {
            return new LocalSleepyVisualState(sleepyStartTick, sleepyStartTick + Math.max(1, asleepDelayTicks));
        }
