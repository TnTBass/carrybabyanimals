...
public final class ModStatusSnapshot {
    ...
    static ModStatusSnapshot withServerVersion(
        ...
        if (status != VersionStatus.MATCHED && status != VersionStatus.BUILD_DIFFERENT && status != VersionStatus.DIFFERENT) {
            throw new IllegalArgumentException("status must be MATCHED, BUILD_DIFFERENT, or DIFFERENT when serverVersion is present");
        }
