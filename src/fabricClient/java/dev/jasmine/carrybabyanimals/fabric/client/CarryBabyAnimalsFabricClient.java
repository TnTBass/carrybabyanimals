package dev.jasmine.carrybabyanimals.fabric.client;
...
import dev.jasmine.carrybabyanimals.fabric.client.render.CarriedBabyRenderer;
...
public final class CarryBabyAnimalsFabricClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(FabricCarryNetworking.SetCarriedPayload.TYPE, (payload, context) ->
                context.client().execute(() ->
                        CarriedBabyRenderState.set(payload.babyEntityId(), payload.carrierEntityId())
                )
        );
        ClientPlayNetworking.registerGlobalReceiver(FabricCarryNetworking.ClearCarriedPayload.TYPE, (payload, context) ->
                context.client().execute(() ->
                        CarriedBabyRenderState.clear(payload.babyEntityId())
                )
        );
        ClientPlayNetworking.registerGlobalReceiver(FabricCarryNetworking.PetFeedbackPayload.TYPE, (payload, context) ->
                context.client().execute(() ->
                        ClientCarryInteractionHandler.onPetFeedback(payload.babyEntityId())
                )
        );
        ClientPlayNetworking.registerGlobalReceiver(FabricCarryNetworking.ServerVersionPayload.TYPE, (payload, context) ->
                context.client().execute(() ->
