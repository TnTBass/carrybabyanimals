...
function Test-ModrinthUploadEnforcesRequiredServerOptionalClient {
    ...
    Assert-Contains $script '$loaderDisplayName = if ($Loader -eq "neoforge") { "NeoForge" } else { "Fabric" }' 'Modrinth upload must derive a friendly loader display name.'
    Assert-Contains $script 'name = "Carry Baby Animals $Version for Minecraft $minecraftVersion ($loaderDisplayName)"' 'Modrinth upload must display friendly per-loader version names.'
    Assert-NotContains $script 'name = $modrinthVersionNumber' 'Modrinth upload must not display the loader-suffixed version number as the public version name.'
